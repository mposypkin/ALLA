...
double GRAD(int MP, int M, int N, TersoffParams tsof, double *x1, double *x2, double *gr_x1, double *gr_x2) {
	...
	double *u; // vektor izmenyayushchihsya parametrov potenciala Tersoffa
    ...
    double **r = new double*[MM]; // rasstoyaniya 
    double **rr = new double*[MM]; // rasstoyaniya 
    double **rrr = new double*[MM]; // rasstoyaniya 
    double **z = new double*[MM]; // fazovye peremennye
    double **v = new double*[MM]; // fazovye peremennye
    double **w = new double*[MM]; // fazovye peremennye
    double **R = new double*[MM]; // fazovye peremennye
    double **T = new double*[MM]; // fazovye peremennye
    double **Q = new double*[MM]; // fazovye peremennye
    double **V = new double*[MM]; // fazovye peremennye
    double **f = new double*[MM]; // Sglazhivayushchaya funkciya f
    double **df = new double*[MM]; // Proizvodnye sglazhivayushchej funkcii f
    double **pf = new double*[MM]; // Sopryazhennye peremennye
    double **pr = new double*[MM]; // Sopryazhennye peremennye
    double **pz = new double*[MM]; // Sopryazhennye peremennye
    double **pv = new double*[MM]; // Sopryazhennye peremennye
    double **pw = new double*[MM]; // Sopryazhennye peremennye
    double **pR = new double*[MM]; // Sopryazhennye peremennye
    double **pT = new double*[MM]; // Sopryazhennye peremennye
    double **pQ = new double*[MM]; // Sopryazhennye peremennye
    double **pV = new double*[MM]; // Sopryazhennye peremennye
    double **d = new double*[MM]; // Rabochij massiv
    ...
    double ***ar = new double**[MM]; // rasstoyaniya 
    double ***br = new double**[MM]; // rasstoyaniya 
    double ***q = new double**[MM]; // ������� ����
    double ***g = new double**[MM]; // fazovye peremennye
    double ***x = new double**[MM]; // fazovye peremennye
    double ***y = new double**[MM]; // fazovye peremennye
    double ***ff = new double**[MM]; // Sglazhivayushchaya funkciya f
    double ***dff = new double**[MM]; // Proizvodnye sglazhivayushchej funkcii f
    double ***pq = new double**[MM]; // Sopryazhennye peremennye
    double ***pg = new double**[MM]; // Sopryazhennye peremennye
    double ***px = new double**[MM]; // Sopryazhennye peremennye
    double ***py = new double**[MM]; // Sopryazhennye peremennye
    double ***pff = new double**[MM]; // Sopryazhennye peremennye
    double ***par = new double**[MM]; // Sopryazhennye peremennye
    double ***pbr = new double**[MM]; // Sopryazhennye peremennye
    double ***tau = new double**[MM]; // rij - rik
    ...
    RC = tsof.mRcut;
    //------------------------------------------------------------------------------
	r1 = u[8]*u[8];   	             r2 = u[9]*u[9];   	       r3 = 1.0 + r1/r2;                r4 = u[7]*u[7]*u[7];
	r5 = -u[3] * sqrt(2.0 * u[4]);   r6 = u[1] / (u[4] - 1);   r7 = u[1] * u[4] / (u[4] - 1);   r8 = -u[3] * sqrt(2.0 / u[4]);
	r9 = 1.0 / u[4] - 1.0;            r15 = -1.0 / 2.0 / u[5];           
    ...
    for (i = 1; i <= IM; i++)
	   ...
	   if (v[i][j] < 1.0e-25) hlp = pow((1.0e-25), (u[5] - 1));  else hlp = pow((v[i][j]), (u[5] - 1));
...
double grad(int nlay, const struct TersoffParams& tsof, const int* lb, const int* ub, const double* x, double* g) {
    int ki, N, M, LM, MP, MM, II, I, k, n, i, j, m, num, ind, *Jz, *Jzz, *im, *jm, *g_s, *g_h, K;
    double aaa, *x1, *x2, *gr_x1, *gr_x2, F, *grad_h, *grad_d, *grad_s, *d, *s, *h, *dm, *dp, xmin, xmax, Sum, RO;
    double RR, RC; // zadannye parametry

    I = nlay;
    II = I + 1;
    N = 10; //   kolichestvo izmenyaemyh parametrov potenciala;
    ...
    if ((d = (double *) malloc(II * sizeof (double))) == NULL) {
        printf("\n  Net pamyati \n");
        exit(10);
    }
    if ((s = (double *) malloc(II * sizeof (double))) == NULL) {
        printf("\n  Net pamyati \n");
        exit(10);
    }
...
//                              kolichestvo atomov ocrujeniya:
	...
	RO = RR + RC;
xmin = d[1] - RO;
xmax = d[1] + (Jzz[1] - Jz[1])*s[1] + RO;
for(i = 2; i <= I; i++)
{if((d[i] - RO) < xmin)       xmin = d[i] - RO;
 if((d[i] + (Jzz[i] - Jz[i])*s[i] + RO) > xmax)       xmax = d[i] + (Jzz[i] - Jz[i])*s[i] + RO;
}
//----------------------------------------------------------------------------
